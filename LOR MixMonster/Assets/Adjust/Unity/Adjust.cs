...
#if UNITY_ANDROID
            appToken = config.sdkIdConfig.adjustID_Android;
#elif UNITY_IOS
                appToken = config.sdkIdConfig.adjustID_Ios;
#endif
            this.environment = config.sdkIdConfig.adjustEnvironment;
#if UNITY_EDITOR
            GameUtility.GameUtility.Log("Adjust id " + appToken + " " + this.environment);
#endif
            if (IsEditor())
            {
                return;
